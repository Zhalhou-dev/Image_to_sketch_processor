...
def water_color_effect(decode_array_to_img):
    water_color_img=cv2.stylization(decode_array_to_img,sigma_s=80,sigma_r=0.4)
    status,output_img=cv2.imencode('.PNG',water_color_img)
    return output_img
...
def hdr_effect(decode_array_to_img):
    hdr_img=cv2.detailEnhance(decode_array_to_img,sigma_s=80,sigma_r=0.2)
